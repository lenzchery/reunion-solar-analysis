...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.21708004.svg)](https://doi.org/10.5281/zenodo.21708004)
...
1. **Clip** the Global Solar Atlas GHI raster (daily average, 1999-2018), already reprojected to the island's target CRS (EPSG:2975) in QGIS, to the administrative extent of Reunion Island.
...
│
├───outputs                  <- regenerated by scripts/01 -> 03, not tracked
│   ├───maps
│   └───tables
│
└───scripts
...
`figures/` holds static copies of the final maps, tracked for display in this README. `outputs/` is where the pipeline writes its results when you run it locally — it is not version-controlled (see `.gitignore`), since it is fully reproducible from `data/` via `scripts/`.
...
|---|---|---|
| `01_reproject_clip.py` | Clips the GHI raster to the extent of Reunion Island using `rioxarray` (`raster.rio.clip()`). The raster's pixel grid is reprojected beforehand in QGIS (WGS 84 → RGR92 / UTM 40S, EPSG:2975); the script only assigns/declares this CRS via `rio.write_crs()` (no pixel resampling in Python) before clipping | `outputs/maps/ghi_reunion_clip.tif`, `outputs/maps/reunion_boundary.gpkg` |
...
**Note on the pipeline:** the raster reprojection step (WGS 84 → RGR92 / UTM 40S, EPSG:2975) was performed once in QGIS prior to committing `GHI_Reunion.tif` to this repository. From that point on, the entire pipeline — clipping, statistics, zone extraction, vectorization, and mapping — runs **entirely in Python** via `rioxarray`, `rasterio`, and `geopandas`.
...
The GHI raster is already included in `data/raster/` (already reprojected to EPSG:2975, see "Methodology and scripts" above). The commune boundary file is **not** bundled (see "Data and Licenses" above) — download it from [purl.stanford.edu/pn417fx4462](https://purl.stanford.edu/pn417fx4462) and place `REU_adm2.shp` (+ `.dbf`/`.shx`/`.prj`) in `data/vectors/`. Then run the scripts in order:
...
<p align="center">
  <img src="figures/Final_GHI_Distribution_Map.png"
       alt="Spatial distribution of Global Horizontal Irradiance across Reunion Island"
       width="700">
</p>

<p align="center">
<em>Spatial distribution of daily average GHI (1999–2018) across Reunion Island, by commune.</em>
</p>

<p align="center">
  <img src="figures/Optimal_Solar_Zones.png"
       alt="Optimal solar cooking zones in Reunion Island, GHI at least 5 kWh per m2 per day"
       width="700">
</p>

<p align="center">
<em>Optimal solar cooking zones (GHI ≥ 5 kWh/m²/day), overlaid with communal boundaries.</em>
</p>
...
> **Chéry, L. A. (2026).** *Reunion Solar Analysis: Spatial Assessment of Global Horizontal Irradiance (GHI) for Solar Cooking Applications* (Version 1.0.3) [Computer software]. Zenodo.
> https://doi.org/10.5281/zenodo.21708004
